...
// Initialize route button functionality
function initializeRouteButton() {
    ...
    // Function to show route button when an establishment is selected
    window.selectEstablishmentForRoute = function(estabData) {
        selectedEstablishment = estabData;
        routeBtn.classList.add('visible');
    };

    // Handle route button click
    routeBtn.addEventListener('click', function() {
        if (!selectedEstablishment || !map || !userLocation) {
            alert('Please wait for your location to be detected and select an establishment');
            return;
        }

        // Toggle active state
        const isActive = routeBtn.classList.toggle('active');
        ...
        if (isActive) {
            ...
            currentRoutingControl = L.Routing.control({
                waypoints: [
                    ...
                    L.latLng(selectedEstablishment.latitude, selectedEstablishment.longitude)
                ],
                routeWhileDragging: false,
                addWaypoints: false,
                draggableWaypoints: false,
