...
class DatabaseErrorMiddleware:
    ...
    def _db_error_response(self, request):
        """
        - Direct browser page visits → skeleton HTML (never raw JSON)
        - AJAX/API/fetch calls       → JSON 503 (JS handles blur in-place)

        Detection priority (most reliable first):
        1. Explicit AJAX header
        2. /api/ path prefix
        3. Accept header does NOT contain text/html  ← catches ALL fetch/XHR calls
        4. If Accept contains text/html → it is a real browser page visit → HTML
        """
        import json
        ...
        is_ajax = (
            ...
            or (
                'text/html' not in accept          # fetch/XHR never sends text/html
                and accept != ''                    # some old clients send empty Accept
            )
        )
        ...
        # Real browser page visit → skeleton HTML, never raw JSON
