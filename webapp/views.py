...
# ============================================================
# ============================================================
# ILAGAY ITO SA DULO NG views.py (bago ang huling linya)
# Siguraduhing nandoon ang tamang imports:
#   from .models import FavoriteEstablishment, FavoriteMenuItem
# ============================================================
...
from django.shortcuts import render, get_object_or_404
from .models import FoodEstablishment, MenuItem, FavoriteEstablishment, FavoriteMenuItem


# ── 1. TOGGLE FAVORITE ESTABLISHMENT (⭐ Star button) ──────────────────────
@login_required
@require_POST
def toggle_favorite_establishment(request):
    """
    Toggle star on an establishment.
    POST body: { "establishment_id": <int> }
    Returns: { "status": "ok", "action": "added"|"removed", "is_favorite": true|false }
    """
    try:
        body   = json.loads(request.body)
        est_id = int(body.get('establishment_id', 0))
    except (ValueError, TypeError, json.JSONDecodeError):
        return JsonResponse({'status': 'error', 'message': 'Invalid request'}, status=400)

    establishment = get_object_or_404(FoodEstablishment, id=est_id, is_active=True, is_approved=True)

    fav, created = FavoriteEstablishment.objects.get_or_create(
        user=request.user,
        establishment=establishment
    )
...
# ── 2. TOGGLE FAVORITE MENU ITEM (❤️ Heart button) ────────────────────────
@login_required
@require_POST
def toggle_favorite_menu(request):
    """
    Toggle heart on a menu item.
    POST body: { "menu_item_id": <int> }
    Returns: { "status": "ok", "action": "added"|"removed", "is_favorite": true|false }
    """
    try:
        body         = json.loads(request.body)
        menu_item_id = int(body.get('menu_item_id', 0))
    except (ValueError, TypeError, json.JSONDecodeError):
        return JsonResponse({'status': 'error', 'message': 'Invalid request'}, status=400)

    menu_item = get_object_or_404(MenuItem, id=menu_item_id)

    fav, created = FavoriteMenuItem.objects.get_or_create(
        user=request.user,
        menu_item=menu_item
    )
...
# ── 3. GET ALL FAVORITE ESTABLISHMENTS (for favorites page) ───────────────
@login_required
@require_GET
def get_favorite_establishments(request):
    """
    Returns the logged-in user's list of favorited establishments
    with enough data to render the favorites page cards.
    """
    favs = (
        FavoriteEstablishment.objects
    ...
    data = []
    for fav in favs:
        est = fav.establishment
        if not est.is_active or not est.is_approved:
            continue
        data.append({
            'id':             est.id,
            'name':           est.name,
            'address':        est.address or '',
            'image':          est.image.url if est.image else '',
            'status':         est.calculated_status.lower(),
            'average_rating': float(est.average_rating) if hasattr(est, 'average_rating') else 0,
            'review_count':   est.review_count if hasattr(est, 'review_count') else 0,
            'payment_methods': est.payment_methods or 'Cash',
            'favorited_at':   fav.created_at.isoformat(),
        })

    return JsonResponse({'status': 'ok', 'establishments': data})


# ── 4. GET ALL FAVORITE MENU ITEMS (for favorites page) ───────────────────
@login_required
@require_GET
def get_favorite_menus(request):
    """
    Returns the logged-in user's list of favorited menu items.
    """
    favs = (
        FavoriteMenuItem.objects
        .filter(user=request.user)
        .select_related('menu_item', 'menu_item__food_establishment')
        .order_by('-created_at')
    )
    ...
    return JsonResponse({'status': 'ok', 'menus': data})


# ── 5. GET FAVORITES STATUS (batch — for syncing star/heart states) ────────
@login_required
@require_GET
def get_favorites_status(request):
    """
    Returns the IDs of all favorited establishments and menu items
    for the current user. Used to pre-fill ⭐/❤️ states on page load.
    """
    fav_est_ids  = list(
        FavoriteEstablishment.objects
        .filter(user=request.user)
    ...
    return JsonResponse({
        'status':            'ok',
        'fav_establishments': fav_est_ids,
        'fav_menus':          fav_menu_ids,
    })


# ── 6. FAVORITES PAGE VIEW ─────────────────────────────────────────────────
@login_required
def favorites_page(request):
    """
    Renders the favorites page (favorites.html).
    The actual data is loaded via JS fetch calls to the API endpoints above.
    """
