...
def get_all_menu_items(request):
    """
    🍽️ ALL MENU ITEMS API - REAL-TIME

    Fetches ALL menu items from ALL establishments in real-time.
    Returns complete menu catalog across KabsuEats with:
    ...
    """
    try:
        # ✅ Get all menu items that are available and belong to active establishments
        all_items = MenuItem.objects.filter(
            quantity__gt=0,  # ✅ Only show items in stock
            food_establishment__isnull=False,  # ✅ Must belong to an establishment
            food_establishment__status__in=['Open', 'Closed']  # ✅ Exclude disabled establishments
        ).select_related(
            'food_establishment',
            'food_establishment__category'
        ).order_by(
            'food_establishment__name',  # ✅ Group by establishment
            'name'  # ✅ Then alphabetically by item name
        )
        ...
        for item in all_items:
            ...
            items_data.append({
                ...
                'category': item.category if hasattr(item, 'category') and item.category else 'Uncategorized',
                'availability': item.quantity > 0,
                'quantity': item.quantity,
                'establishment_id': establishment.id,
                'establishment_name': establishment.name,
                'establishment_category': establishment.category.name if establishment.category else 'Other',
                'establishment_image': establishment.image.url if establishment.image else None,
            })
