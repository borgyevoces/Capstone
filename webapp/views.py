...
# ===== CUSTOMER RECORDS API ENDPOINTS =====

@login_required
@require_http_methods(["GET"])
def get_establishment_orders(request, establishment_id):
    """
    Get all orders for an establishment with filtering options
    Parameters:
        - establishment_id: ID of the food establishment
        - status: (optional) Filter by order status (PENDING, PAID, PREPARING, READY, COMPLETED, CANCELLED)
    Returns JSON with orders and pending count
    """
    try:
        # Get the establishment
        establishment = FoodEstablishment.objects.get(id=establishment_id)
        ...
        for order in orders:
            ...
            orders_data.append({
                ...
                'items': items,
                'rating': 0,  # Add rating if you have reviews
            })
...
def mark_order_as_paid(request, order_id):
    ...
    try:
        ...
        # Create notification
        order.create_notification('payment_confirmed')

        logger.info(f"Order {order_id} marked as paid by {request.user.username}")
...
def update_order_status(request, order_id):
    ...
    try:
        ...
        order.create_notification(notification_type_map.get(new_status, 'new_order'))
...
@login_required
@require_http_methods(["GET"])
def get_transactions_history(request, establishment_id):
    """
    Get transaction history for an establishment
    """
    try:
        establishment = FoodEstablishment.objects.get(id=establishment_id)
        ...
        for order in orders:
            transactions.append({
                ...
                'description': f'Order from {order.user.username}',
                'status': order.status,
                'created_at': order.created_at.isoformat(),
                'payment_confirmed_at': order.payment_confirmed_at.isoformat() if order.payment_confirmed_at else None,
            })
    ...
    try:
        ...
        return JsonResponse({
            ...
            'customer': {
                ...
                'member_since': customer.date_joined.isoformat(),
            }
        })
...
@login_required
@require_http_methods(["GET"])
def get_order_statistics(request, establishment_id):
    """
    Get statistics about orders for the establishment
    """
    try:
        establishment = FoodEstablishment.objects.get(id=establishment_id)
